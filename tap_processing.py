...
def get_dataset(number_trajectories,satart_number_ts,number_ts,per_t,batch_size,dataset_dir,path):
  ...
  for i in range(number_trajectories):
    ...
    if os.path.isdir(path+str(i)):
      path_k=path+str(i)
      print(path_k)
      vtu_num=get_vtu_num(path_k)
      print(vtu_num)
      for ts in range(satart_number_ts,vtu_num):
        ...
        if((ts-satart_number_ts)%(batch_size*per_t)==0):
          ...
          for i in range(batch_size*per_t):
            if i==0:
              ...
              #temperature = mesh.point_data["Temperature"][:]  # 温度
              #temperature=temperature.reshape(-1,1)
              #temperature = torch.tensor(temperature).type(torch.float)
            ...
            if i>=1:
              ...
              velocity1 = mesh.point_data["Velocity"][:, [0,1]]  # u,v方向速度
              velocity1 = torch.tensor(velocity1).type(torch.float)
              #temperature1 = mesh.point_data["Temperature"][:]  # 温度
              #temperature1=temperature1.reshape(-1,1)
              #temperature1 = torch.tensor(temperature1).type(torch.float)

              #triangle_cells = mesh.get_cells_type('triangle')
              #edges = triangles_to_edges(tf.convert_to_tensor(triangle_cells))
              #edge_index1 = torch.cat( (torch.tensor(edges[0].numpy()).unsqueeze(0) ,
                         #torch.tensor(edges[1].numpy()).unsqueeze(0)), dim=0).type(torch.long)
              #mesh_pos1 = mesh.points[:,[0,1]]
              #u_i=torch.tensor(mesh_pos1)[edge_index1[0]]
              #u_j=torch.tensor(mesh_pos1)[edge_index1[1]]
              #u_ij=u_i-u_j
              #u_ij_norm = torch.norm(u_ij,p=2,dim=1,keepdim=True)
              #edge_attr1 = torch.cat((u_ij,u_ij_norm),dim=-1).type(torch.float)
              #cells1=torch.tensor(triangle_cells)
              #mesh_pos1=torch.tensor(mesh_pos1)
              velocity=torch.vstack((velocity, velocity1))
              #temperature=torch.vstack((temperature,temperature1))
              #edge_index=torch.vstack((edge_index,edge_index1))
              #edge_attr=torch.vstack((edge_attr,edge_attr1))
              #cells=torch.vstack((cells,cells1))
              #mesh_pos=torch.vstack((mesh_pos,mesh_pos1)).type(torch.float)
          #data_list.append(Data(x=temperature, edge_index=edge_index, edge_attr=edge_attr,
