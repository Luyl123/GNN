...
from poolings_duibishiyan import model_1_TopKPooling_Mod, avg_pool_mod, avg_pool_mod_no_x
    ...
    def forward(self, x, edge_index, edge_attr, pos, batch=None):
        ...
        for m in range(1, self.depth + 1):
            ...
            if self.interpolation_mode == 'learned':
                pos_f = pos.clone()
                edge_index, edge_attr, batch, pos, cluster, perm = avg_pool_mod_no_x(cluster,edge_index, edge_attr,batch, pos)
                
                n_nodes = x.shape[0]
                edge_index_f2c = torch.cat( (torch.arange(0, n_nodes, dtype=torch.long, device=x.device).view(1,-1), cluster.view(1,-1)), axis=0 )
...
class model_1_GNN_TopK(torch.nn.Module):
    def __init__(self, 
        ...
        for i in range(self.depth):
            self.pools.append(model_1_TopKPooling_Mod(hidden_channels, self.pool_ratios[i]))
    ...
    def forward(self, data,per_t , batch_size,mean_vec_x,std_vec_x,mean_vec_edge,std_vec_edge,batch=None, return_mask=True):
...
class model_2_GCN_Unet(torch.nn.Module):
    def __init__(self,
                 hidden_channels: int, 
                 in_channels_node : int, 
                 in_channels_edge : int, 
        ...
        self.pools = torch.nn.ModuleList() # for pooling 
        for i in range(self.depth):
            self.pools.append(model_1_TopKPooling_Mod(hidden_channels, self.pool_num[i]))

            
        # Reset params 
        self.model_2_reset_parameters()
    ...
    def model_2_reset_parameters(self):
        # Node encoding
        for module in self.node_encode:
            module.reset_parameters()
